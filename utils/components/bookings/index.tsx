    ...
    <div className="min-h-screen mt-10 space-y-10">
      ...
      <div className="sm:w-1/2 mx-auto">
        ...
        <dialog open={openModal} className="z-20">
          <button onClick={handleCloseModal} className="float-right text-secondary">X</button>
          <h3>Booking</h3>
          <p>Fill the form below to create a booking:</p>
          <AddBookingForm handleCloseModal={handleCloseModal} />
        </dialog>
        <button onClick={handleOpenModal} className="btn btn-sm border-0 bg-primary my-5">Add Booking</button>
        <BookingsTable />
      </div>
    </div>
  );
